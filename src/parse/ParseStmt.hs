...
parseStmt :: Preprocess.ProcTypeMap -> [Tokens.Token] -> (Ast.Stmt, [Tokens.Token])
parseStmt procMap (Tokens.Ident name : Tokens.Equal : tks) =
...
parseStmt _ (Tokens.Return : Tokens.Semicolon : tks) = (Ast.Return Nothing, tks)
parseStmt procMap (Tokens.Return : tks) =
  let (expr, tks2) = ParseExpr.parseExpr procMap tks
   in case tks2 of
        (Tokens.Semicolon : tks3) -> (Ast.Return (Just expr), tks3)
        _ -> error ("expected semicolon, got=" ++ show (head tks2))
parseStmt procMap tokens@(Tokens.LBrace : _) = let (stmts, tks) = parseBlock procMap tokens in (Ast.BlockStmt stmts, tks)
parseStmt procMap tokens@(Tokens.When : _) = parseWhen procMap tokens
parseStmt procMap tokens@(Tokens.While : _) = parseWhile procMap tokens
parseStmt procMap tokens =
...
parseBlock :: Preprocess.ProcTypeMap -> [Tokens.Token] -> ([Ast.Stmt], [Tokens.Token])
parseBlock procMap (Tokens.LBrace : tokens) = helper tokens []
 where
  helper :: [Tokens.Token] -> [Ast.Stmt] -> ([Ast.Stmt], [Tokens.Token])
  helper (Tokens.RBrace : tks) acc = (reverse acc, tks)
  helper tks acc =
    let (stmt, tks') = parseStmt procMap tks
     in helper tks' (stmt : acc)
parseBlock _ tokens = error ("expected LBrace, got=" ++ show (head tokens))
...
parseWhen :: Preprocess.ProcTypeMap -> [Tokens.Token] -> (Ast.Stmt, [Tokens.Token])
parseWhen procMap (Tokens.When : tks) =
  let (expr, tks2) = ParseExpr.parseExpr procMap tks
   in case tks2 of
        (Tokens.Then : tks3) ->
          let (tStmt, tks4) = parseStmt procMap tks3
           in case tks4 of
                (Tokens.Otherwise : tks5) ->
                  let (fStmt, tks6) = parseStmt procMap tks5
...
parseWhen _ tokens = error ("expected When, got=" ++ show (head tokens))
...
parseWhile procMap (Tokens.While : tks) =
  let (expr, tks2) = ParseExpr.parseExpr procMap tks
   in case tks2 of
        (Tokens.Then : tks3) ->
          let (innerStmt, tks4) = parseStmt procMap tks3
